...
+default::thing(X, Y, entity, Team)
	: not(X == 0 & Y == 0) & default::team(Team) & not(action::reasoning_about_belief(identification)) & default::step(ID) //& identification::identified(List) & common::all_names_new(Ags) & .length(Ags,NumberAgents) & not .length(List,NumberAgents-1)
<-
